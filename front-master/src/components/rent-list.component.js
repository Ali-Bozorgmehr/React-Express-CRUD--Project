...
export default class RentList extends Component {
  ...
  retrieveHouses() {
    HouseDataService.getAllRentsAuth()
      .then((response) => {
        this.setState({
  ...
  removeAllHouses() {
    HouseDataService.deleteAllRentsAuth()
      .then((response) => {
        console.log(response.data);
        this.refreshList();
      })
  ...
  searchOwner() {
    HouseDataService.findRentByOwner(this.state.searchOwner)
      .then((response) => {
        this.setState({
          houses: response.data,
        });
            ...
            {currentHouse ? (
              <div className="mt-3 fs-5">
                ...
                <div>
                  ...
                  {currentHouse.rented ? "Rented" : "Pending"}
                </div>
                ...
                >
                  <Link
                    className="link-light text-decoration-none"
                    to={"/auth/dashboard/rents/" + currentHouse.id}
                  >
                    View and Edit
                  </Link>
                </Button>
